...
- Status: done 2026-07-22 -> eda/interface/{run_fullstack.py,fullstack_summary.csv,fullstack_effects.csv,fullstack_config.json,results_fullstack/}, circuit_backends FullStackSpin (kind full_stack, selfcheck asserts bit-identity with Ideal/Behavioral/CircuitChain), 3.4.2/3.5.2/3.5.3/3.5.6/3.6 (commit 76c1705). OUTCOME: (a) decomposition VALID at the ch2 operating point (interaction 0.63x [0.20,1.93] ER14, 1.63x [0.80,3.34] G1, both contain 1) but only as a LOWER BOUND — three device knobs off-nominal + four circuit channels gives 0/1000 on G1 vs product-predicted p_s=0.087 (>400x super-multiplicative). (b) res-IV screen: rail x h_off SYNERGISTIC 2.71x [2.06,3.71] — FALSIFIES RX-04's "h_off relieved at array scale" (that OFAT had unclipped drive; behind a rail it is 3.02x avg, 8.16x at ±8V_T vs 1.11x at ±12V_T, disjoint). rail x read-flip ANTAGONISTIC 0.71x (damage saturates). Main effects G1: flip 5.03x, rail 4.37x, h_off 3.02x resolved; resetk/bits/CV not. (c) joint PDK mismatch DONE: device_model has NO per-device threshold-shift channel; V_th spread 6.0 V_T, offset-only = 0/200 on G1, per-cell trim recovers to 1.51x but ±3sigma needs 57/63 codes at ±10V_T/6-bit => COMPETES with the IR predistortion code budget (now in 3.5.3). Slope spread is NOT resolved by the source scan (step 62.1 mV > mean width 57.9 mV) — CV 27.6% is an upper bound, correlation untrustworthy. NOTE for RX-10/RX-11: use FullStackSpin; and any future "device knob" claim must state the rail it was measured behind.
...
- Status: done 2026-07-23 -> eda/testbenches/{sigmoid_emergence_llg.py,sigmoid_emergence_llg_summary.json}, §3.5.6 replay-para validating sentence + broadened LLG caveat (commit 159ec07). OUTCOME: DECISION = "none" (no delivered-probability bias) -> validating sentence, NOT a §3.5.1 correction. Method: macrospin LLG (Linux-OSDI vgsot_llg.va + Brown-1963 thermal field), 800 seeds x 12 codes, P->AP; ALL P_sw SIMULATED, compared to harness static sigmoid at the SAME DC flat-top. SCOPING NOTE (important): the plan said "drive the measured V_wr(t) RC pulse", but the deck uses an IDEAL rectangular pulse at the DC-measured v_wr (no buffer/RC) — so RX-12 does NOT exercise the RC un-settled offset; that is RX-13's result. Finding: LLG under-switches vs the static (measured-calibrated) sigmoid, systematic in mid/high drive (max |dp|=0.541 @ code 51; code 30 emp 0.19 vs static 0.49; code 63 emp 0.57 vs static 0.99). Affine-recalibratable: u_emp=0.431u-1.536, max resid 0.204 u (~2.3x wider window). Switching completes in the 1.5 ns post-pulse relaxation tail (p_sw_pulse_end ~0.07 vs final 0.57 @ top code) -> finite-time (tw~tau0). Why no bias: the harness draws Bernoulli in Python from the ch2 MEASURED-calibrated sigmoid (ground truth); the LLG deviation is a macrospin-model + finite-pulse-width effect and appears even under a perfect flat-top; the RX-13 <=5.8 mV (0.25 V_T) un-settled offset is both absent from this deck and too small/wrong-sign (moves P +0.06 max) to explain a 0.54 negative gap. Adversarial-verified (2 auditors): fixed residual <=0.20->~0.20 (true 0.2036), scoped "系统性偏低"->"中高驱动区" (code 7 dev +0.013), disclosed ideal-flat-top + added the magnitude argument so the "not a transient bias" claim is self-supporting. Effort: 2-3 days. Deps: RX-05a harness.
