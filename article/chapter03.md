...
Max-Cut取$J_{ij}=-w_{ij}/2$、$h_i=0$，以公开G-set数据与BKS作为参照[^Gset]。三个实例分别覆盖稠密正权图$\mathrm{G1}$ ($n=800$、$m=19176$)、稀疏符号混合图$\mathrm{G14}$ ($n=800$、$m=4694$) 和更大规模的$\mathrm{G22}$ ($n=2000$、$m=19990$)。求解器采用严格异步Gibbs更新、$T=10000$扫数与$(\beta_0,\beta_f)=(0.1,10.0)$的几何退火，每例运行$N_{\text{trial}}=200$次。BKS与BLS参照数据取自公开汇总[^BenlicHao2013]。
...
三类景观给出不同的命中形态。$\mathrm{G1}$以$p_s=0.685$稳定命中BKS；$\mathrm{G22}$虽只有$4$次精确命中，中位相对偏差仍为$0.022\%$，分布中的次峰揭示了与最优盆地相邻的稳定吸引域。$\mathrm{G14}$在$200$次试验中没有命中BKS，但最佳与中位相对偏差仅$0.065\%$和$0.326\%$，表明整体解质量仍高，困难集中在近优盆地之间的最后跃迁[^process-g14-budget]。其稀疏符号混合结构形成大量切分值仅差$2$—$4$的浅局部极小，低温阶段的单自旋转移难以跨越这些盆地。
...
BLS与p-bit SA只用于量级校准。Benlic和Hao的BLS在不同处理器上报告$\mathrm{G1}$约$2$—$3$秒、$\mathrm{G14}$约$119$秒、$\mathrm{G22}$约$560$秒达到BKS；本节对应值为$3.73$秒、未命中和$335.6$秒。处理器、停止条件与算法均不同，因此这些墙钟数值不能构成速度排名，但足以显示$\mathrm{G14}$对基础退火更困难。Onizawa和Hanyu的pSA在$\mathrm{G1}$上报告归一化切分均值$0.996$—$0.998$，TApSA提高到$0.999$以上[^Onizawa2024]；本节严格异步更新的$\Delta_{\text{med}}=0$处于同一高质量区间。跨论文协议差异不支持把改进单独归因于更新调度，算法因果判断以后续同框架对照为准。
...
单自旋QUBO基线把TSP的结构性边界直接显示出来：可行构型只占$n!/2^{n^2}$，一旦单自旋翻转会破坏one-hot约束，退火就难以在可行巡回路径之间移动。三个实例的可行样本因而停留在最优值的$1.5\times\sim2.5\times$附近。作为外部参照，D-Wave混合求解器Kerberos在`ulysses16`的六次运行中取得$7314$的最佳长度，相对最优值$6859$仍有$6.6\%$偏差[^Warren2021]；其混合搜索明显优于本节基础SA，但同样未消除QUBO编码的搜索代价。
...
硬件含义也因此发生变化：单自旋p-bit阵列给出QUBO基线，置换更新则要求控制层生成2-opt、swap或insert提议、计算$\Delta L$并协调多自旋状态变化。表3.6描述的是这种扩展后的算法上界，不能直接等同于现有单自旋更新链的性能。后续器件消融以Max-Cut和整数分解为主，TSP用于界定单自旋抽象的边界并提出簇更新接口需求。
...
该优势的物理来源是把随机采样从数字状态机下沉到本征随机器件：sMTJ以单个写脉冲产生条件样本，数字平台还需完成随机数、阈值判决与状态更新。表3.8因此给出采样单元的理想下界，尚未计入sMTJ和CMOS p-bit两行的局部场求和、权重存取、驱动与读出；FPGA和CPU两行则已包含这些系统开销。跨平台绝对数字只有在口径一致时才具有优劣含义，3.5节将用晶体管级更新链修正sMTJ一行。

比较范围限于电学Ising硬件。光学相干Ising机与量子退火的能量边界和并行语义不同，不纳入表3.8；其总体脉络参见相关综述[^MohseniReview2022]。本节可迁移的结论是器件级采样具有低能量潜力，阵列级优势仍取决于外围开销能否保持在同一量级。
...
本节结论的适用口径如下。全部CMOS数字为sky130原理图级仿真，主口径取tt角，五个工艺角的散布见表3.9注与3.5.4节，未含版图寄生 (写线电阻已提取，其电容仅用于3.5.3节的RC建立核验、未纳入各单元的能量核算)；失配统计只作用于读出比较器 (见3.5.5节)，写通路CMOS器件的失配未扫描、其影响预期与该处同向 (器件侧的阈值离散另见3.5.3节)；读出失配采用$A_{VT}=5\,\mathrm{mV}\cdot\mu\mathrm{m}$的sky130量级假设而非PDK统计模型，且只注入输入对与锁存对四管，$\sigma_\mathrm{off}$为全锁存失调的下界，可迁移量是$\sigma_\mathrm{off}/V_T$与$\sigma_\mathrm{off}/$裕度而非绝对mV值；缓冲器与使能门取首轮可工作尺寸，未经速度或功耗优化，2.87 mW静态功率与9.0 ns建立时间 (tt角) 应视为该拓扑在此尺寸下的代表值而非极限；复位脉冲需要反极性驱动，其能量此前以同幅值正脉冲的欧姆耗散作代理，现已用最小sky130 H桥实测：双极复位每脉冲约1.48 pJ、较代理高约$19\%$ (增量来自第二只串联开关、右臂直通与栅驱动，桥的空闲静态功耗可忽略)，使端到端$e_\mathrm{update}$在推荐的$k=2\sim3$下升约$4\sim5\%$、G22投影的能耗修正系数自$18.7\times$移至约$19.7\times$，时间轴因复位在0.75 ns脉内建立而不变[^process-hbridge]；复位脉冲序列的统计与写脉冲翻转概率映射均已用宏自旋LLG复核 (分别见3.5.2节与上文)，两项皆属仿真而非实测：前者限于单一工作点、回跳概率对工作点的依赖未扫描，后者以理想平顶脉冲激励、在中高驱动区相对实测标定的静态sigmoid偏低 (可仿射重标定)，测试台的工作点概率因此取自第二章实测标定而非宏自旋仿真；误读通道以独立同分布翻转建模，低估静态失配的实际危害 (见3.5.5节)；局部场$h_i^\mathrm{eff}$的数字求和已按sky130标准单元实测并计入同口径比较 (见3.5.4节)，但该实测仅覆盖累加器本身，权重存储与取数的开销未建模；表3.8中sMTJ与CMOS p-bit两行的原始数值仍保留缺失该项的口径 (FPGA与CPU两行本已含该项)，以便与3.4.3节对照。绝对数值受上述近似影响，以$V_T$归一的比值与各项排序则是可迁移的结论。除3.5.5节标注的外部参照外，本节全部数据由仓库`eda/`目录下的测试台脚本生成，每个数字对应一份带随机种子记录的结果文件，图3.9至图3.11可由`figs_make.py`直接重建。
