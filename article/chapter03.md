...
这一放松有一个前提：本节的驱动不受限幅。把偏置置于3.5节实测的有限基准轨之后，$h_\mathrm{off}=0.1$在$\mathrm{G1}$上的代价回到$3.02\times$ (区间$[2.29,4.10]$、与本节的$1.39\times$不重叠)，在$\pm8V_T$的窄轨下更达$8.16\times$，而$\pm12V_T$下降到$1.11\times$。偏置消耗的是钳位之后剩余的驱动裕度，其危害因而由轨宽预算而非局部场量级单独决定，本节的排序须与3.5.2节的轨宽结论合看。同理，本节的CV$(\Delta)$只刻画增益离散：五参数模型没有逐器件阈值平移这一通道，而按第二章工艺变异数据反推，阈值分布的标准差达$6.0V_T$，单独注入即使$\mathrm{G1}$的200次试验零命中，须由逐单元校准码吸收 (见3.5.3节)。面向阵列的器件优先级据此收敛为：抑制back-hopping平台居首，其次是控制阵列级离散——增益离散CV$(\Delta)$与更棘手的阈值离散；写入偏置与循环噪声在无限驱动裕度下不构成灾难性失效，但偏置的实际代价取决于电路留出的轨宽余量。
