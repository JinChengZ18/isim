...
本节结论的适用口径如下。全部CMOS数字为sky130原理图级仿真，主口径取tt角，五个工艺角的散布见表3.9注与3.5.4节，未含版图寄生 (写线电阻已提取，其电容仅用于3.5.3节的RC建立核验、未纳入各单元的能量核算)；失配统计只作用于读出比较器 (见3.5.5节)，写通路CMOS器件的失配未扫描、其影响预期与该处同向 (器件侧的阈值离散另见3.5.3节)；读出失配采用$A_{VT}=5\,\mathrm{mV}\cdot\mu\mathrm{m}$的sky130量级假设而非PDK统计模型，且只注入输入对与锁存对四管，$\sigma_\mathrm{off}$为全锁存失调的下界，可迁移量是$\sigma_\mathrm{off}/V_T$与$\sigma_\mathrm{off}/$裕度而非绝对mV值；缓冲器与使能门取首轮可工作尺寸，未经速度或功耗优化，2.87 mW静态功率与9.0 ns建立时间 (tt角) 应视为该拓扑在此尺寸下的代表值而非极限；复位脉冲需要反极性驱动，其能量此前以同幅值正脉冲的欧姆耗散作代理，现已用最小sky130 H桥实测：双极复位每脉冲约1.48 pJ、较代理高约$19\%$ (增量来自第二只串联开关、右臂直通与栅驱动，桥的空闲静态功耗可忽略)，使端到端$e_\mathrm{update}$在推荐的$k=2\sim3$下升约$4\sim5\%$、G22投影的能耗修正系数自$18.7\times$移至约$19.7\times$，时间轴因复位在0.75 ns脉内建立而不变[^process-hbridge]；复位脉冲序列的统计已用宏自旋LLG标定 (见3.5.2节)，该标定属仿真而非实测、限于单一工作点，回跳概率对工作点的依赖未扫描；误读通道以独立同分布翻转建模，低估静态失配的实际危害 (见3.5.5节)；局部场$h_i^\mathrm{eff}$的数字求和已按sky130标准单元实测并计入同口径比较 (见3.5.4节)，但该实测仅覆盖累加器本身，权重存储与取数的开销未建模；表3.8中sMTJ与CMOS p-bit两行的原始数值仍保留缺失该项的口径 (FPGA与CPU两行本已含该项)，以便与3.4.3节对照。绝对数值受上述近似影响，以$V_T$归一的比值与各项排序则是可迁移的结论。除3.5.5节标注的外部参照外，本节全部数据由仓库`eda/`目录下的测试台脚本生成，每个数字对应一份带随机种子记录的结果文件，图3.9至图3.11可由`figs_make.py`直接重建。
