...
注：每个实例$N_{\text{trial}}=200$次独立试验，$T=10000$扫数，$(\beta_0,\beta_f)=(0.1, 10.0)$，JIT编译的异步Gibbs更新。$\Delta_{\text{best}}$与$\Delta_{\text{med}}$为最佳与中位切分相对于BKS的相对偏差。表中各$p_s$的Wilson区间按本节开头的统计约定计算，逐行数值见仓库`eda/interface/ci_audit_summary.csv`。
...
把同一组扫描搬到$\mathrm{G1}$ ($n=800$、平均度$47.9$，G-set协议$T=10^4$、$\beta_f=10$、$N_\mathrm{trial}=200$) 之后，排序的两端稳定而中段重排。稳定的两端是：$p_\mathrm{max}$仍最具破坏性，且在阵列规模下更甚——$0.9$即使$200$次试验无一命中 (14自旋实例上尚为$10\times$)，能量中位数自$-2036$坍缩至$-1171.5$；$\sigma_\mathrm{C2C}$仍最被容忍，$\sigma=2$下退化$1.04\times$、落在基线的Wilson区间内。重排出现在中段：D2D离散升为第二约束，$\mathrm{CV}=0.30$在$\mathrm{G1}$上产生统计可分辨的$1.66\times$退化 ($p_s$自$0.720$降至$0.535$，区间不重叠)，而在14自旋实例上仅$1.3\times$；写入偏置$h_\mathrm{off}$则相反，$0.2$在14自旋实例上的$40\times$灾难不复现，$\mathrm{G1}$上退化$1.25\times$且未达统计分辨。机理在于$h_\mathrm{off}$以$\max|J|$为单位定义，而典型$|h_i^\mathrm{eff}|$随连接度增长，同一绝对偏置在度$48$的图上只是小扰动。$g_\mathrm{dev}>1$的加速同样不外推 ($1.5$倍增益在$\mathrm{G1}$上为$1.16\times$、与基线不可分辨)，故前段“略高于理想增益的工作点有利于求解”只在低连接度景观上成立。面向阵列的器件优先级据此收敛为：抑制back-hopping平台居首，其次是控制晶圆级增益离散，写入偏置与循环噪声在阵列规模下均获得相对放松。
