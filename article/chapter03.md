## 第三章 基于sMTJ的伊辛求解仿真框架与性能评估

本章在前文对sMTJ器件物理、概率输出特性与阵列实现约束的讨论基础上，面向组合优化求解建立从问题编码到性能投影的仿真评估链路。首先将Max-Cut、旅行商问题与整数分解统一表述为伊辛哈密顿量最小化问题，并以5节点Max-Cut实例展示“问题—编码—采样—退火—解码”的完整流程；随后基于公共基准与消融实验评估理想sMTJ-Gibbs动力学的求解能力、局限与改进方向；最后将器件级非理想性和跨硬件时间—能耗模型纳入同一框架，为后续在算法指标、器件参数与架构设计之间进行取舍提供可复现的数据依据。
...
综合五组扫描结果，求解性能对$g_\mathrm{dev}$、$\sigma_\mathrm{C2C}$与$p_\mathrm{max}$敏感，而对$h_\mathrm{off}$与$\mathrm{CV}(\Delta)$较为宽容。这一灵敏度排序与电学测量的器件级关注点恰好相反——存储MRAM苛求$\mathrm{CV}(\Delta)$与$h_\mathrm{off}$的低离散度，sMTJ作为概率求解器原语却在这两项上有充裕的容差余量，其主要约束转移到$g_\mathrm{dev}$标定与back-hopping抑制两条路径上。这一定量观察为后续器件级工艺优化与电路级写入策略设计提供了明确的优先级依据。
...
本章围绕sMTJ伊辛求解建立了贯通问题编码、求解动力学、器件非理想性与硬件投影的统一接口。前端以$(J,h)$承接各类组合优化问题；求解层共享异步Gibbs内核、几何退火与由`SeedSequence`派生的种子序列；器件层以五参数行为模型$(g_\mathrm{dev},h_\mathrm{off},\sigma_\mathrm{C2C},p_\mathrm{max},\mathrm{CV}(\Delta))$描述非理想性；硬件层则通过$(t_\mathrm{update},e_\mathrm{update},N_\parallel)$将不同平台投影为可比较的TTS与每解能耗。由此，sMTJ作为概率求解原语的作用不再停留于经验判断，而可在采样规则、退火参数、器件扰动与硬件实现四个层面分别检验并追溯差异来源。
...
器件与硬件分析揭示了灵敏度结构的转移。MRAM作为存储元件时通常强调均匀性（低$\mathrm{CV}(\Delta)$）与对称性（低$h_\mathrm{off}$），但五参数消融显示，当sMTJ被用作概率求解原语时，这两项反而具有较宽容差；back-hopping平台$p_\mathrm{max}$与驱动增益$g_\mathrm{dev}$的标定则更直接地影响求解性能。这一结果说明，面向概率计算的sMTJ优化目标并不等同于存储MRAM的优化目标。跨硬件投影进一步表明，sMTJ-array在能耗上的领先主要来自采样机制本身的物理迁移，即将随机更新从数字状态机下沉到本征随机器件层，而非来自更激进的外围电路设计。因此，本章给出的不仅是一组benchmark结果，也为器件筛选、工作点标定与系统选型提供了优先级依据。

本章仍存在三方面局限。首先，置换空间求解器虽然给出了TSP性能可达上界，但其所需的多p-bit time-aligned驱动尚未落实为sMTJ阵列电路方案。其次，五参数行为模型主要描述稳态非理想性，尚未覆盖累积写入退化与$1/f$低频漂移，因此本章关于工艺裕度的判断仍以器件出厂初期为前提。最后，硬件投影的比较范围限于纳秒级、皮焦级的电学Ising硬件，尚未纳入光学相干Ising机和量子退火等异类平台。后续章节将复用本章建立的器件级Sigmoid接口、行为模型与硬件投影方法，并在PBNN场景下继续考察其系统意义。
