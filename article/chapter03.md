...
把两个单元级平台放到同一口径下，这一修正的读法才完整。表3.8的sMTJ与CMOS p-bit两行都不含局部场$h_i^\mathrm{eff}$的数字求和，而FPGA与CPU两行含。用sky130标准单元实测该项：12位加法器每次累加$995\,\mathrm{fJ}$、二值popcount计数器$469\,\mathrm{fJ}$，其中$389\,\mathrm{fJ}$是时钟树与寄存器的固定开销、与操作数无关[^process-synapse]。按各实例的平均度折算 ($\mathrm{G1}$为$47.9$、$\mathrm{G22}$为$20.0$)，突触项即达$7.3\sim17.6\,\mathrm{pJ}$，本身就超过两行各自的单元能量。把它同时加到两行之后，$\mathrm{G22}$上sMTJ端到端与CMOS p-bit的能耗比自$2.94\times$收窄到$1.79\times$，$\mathrm{G1}$上为$1.43\times$，在耦合稠密的TSP编码上更收窄到$1.13\sim1.15\times$：3.5.4节此前观察到的能耗落差，多半来自两行共同缺失的一项，而非平台差异。

[^process-synapse]: 过程记录：突触项原拟沿用第四章测得的每次操作$19.4\,\mathrm{fJ}$，但在本流程内实测同一操作为$367\,\mathrm{fJ}$。差异不在器件模型，而在口径——数据翻转估计不计入每次累加都要支付的时钟与寄存器开销，该项占$389\,\mathrm{fJ}$。沿用原常数会把口径修正低估约$20\times$。实测台带功能自校验：曾有一版时钟沿错配的数据通路给出看似合理的$725\,\mathrm{fJ}$，但寄存器终值与软件累加器不符，由该校验拦下。

口径一致并不改变排序，改变排序需要驱动侧工程。突触项同时加于两行、在不等式中相消，故交叉点与该项无关；并行度$N_\parallel$对能耗为中性，也不改变比值。真正的可调量是驱动共享：设$S$列共用一套DAC与缓冲器，sMTJ端到端能耗重回CMOS p-bit之下所需的$S$为$k=1$时$\geq1.9$、$k=2$时$\geq5.5$、$k=3$时$\geq102$，$k\geq4$在任何$S$下不可达。代价在时间轴：共享使建立时间按$S$串行化，相对CMOS的$\mathrm{TTS}$自$S=1$的$2.4\times$升至$S=4$的$7.8\times$、$S=16$的$29.5\times$。这条曲线把此前定性的时间-能耗权衡变成可计算的取舍，同时暴露一处内部张力：3.5.2节由复位可靠性推荐的$k\approx2\sim3$，其中$k=3$所需的$102$路共享并不现实，能耗与可靠性两侧的最优$k$并不重合，实际设计须在$k\leq2$上取平衡。若3.5.5节的读出比较器还须放大以获得判决裕度 (失调自$18.5$降到$3.3\,\mathrm{mV}$需面积约$30\times$)，读能量自$0.12$升至$3.23\,\mathrm{pJ}$，此时任何$(k,S)$组合都无法达成能耗持平；先以自调零结构改善再放大，则$k\leq2$下仍有解。
...
本节结论的适用口径如下。全部CMOS数字为sky130原理图级仿真，主口径取tt角，五个工艺角的散布见表3.9注与3.5.4节，未含版图寄生 (写线电阻除外)；失配统计只作用于读出比较器 (见3.5.5节)，写通路CMOS器件的失配未扫描、其影响预期与该处同向 (器件侧的阈值离散另见3.5.3节)；读出失配采用$A_{VT}=5\,\mathrm{mV}\cdot\mu\mathrm{m}$的sky130量级假设而非PDK统计模型，且只注入输入对与锁存对四管，$\sigma_\mathrm{off}$为全锁存失调的下界，可迁移量是$\sigma_\mathrm{off}/V_T$与$\sigma_\mathrm{off}/$裕度而非绝对mV值；缓冲器与使能门取首轮可工作尺寸，未经速度或功耗优化，2.87 mW静态功率与9.0 ns建立时间 (tt角) 应视为该拓扑在此尺寸下的代表值而非极限；复位脉冲需要反极性驱动，以同幅值正极性脉冲的欧姆耗散作其能量代理，桥式驱动电路未实现；复位脉冲序列的统计已用宏自旋LLG标定 (见3.5.2节)，该标定属仿真而非实测、限于单一工作点，回跳概率对工作点的依赖未扫描；误读通道以独立同分布翻转建模，低估静态失配的实际危害 (见3.5.5节)；局部场$h_i^\mathrm{eff}$的数字求和已按sky130标准单元实测并计入同口径比较 (见3.5.4节)，但该实测仅覆盖累加器本身，权重存储与取数的开销未建模；表3.8各行的原始数值仍保留缺失该项的口径，以便与3.4.3节对照。绝对数值受上述近似影响，以$V_T$归一的比值与各项排序则是可迁移的结论。除3.5.5节标注的外部参照外，本节全部数据由仓库`eda/`目录下的测试台脚本生成，每个数字对应一份带随机种子记录的结果文件，图3.9至图3.11可由`figs_make.py`直接重建。
