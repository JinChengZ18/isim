...
图3.10 写链实测约束回灌到3.4.2节消融框架的结果。(a) DAC位宽扫描：固定基准轨与$\beta$缩放基准轨两种退火实现下的$\mathrm{TTS}_{99}$退化比，星号为实测6-bit网格；(b) 钳位窗口随实例规模的移动：ER曲线为五个种子的几何均值，$\mathrm{G1}$为G-set协议单次运行，倒三角标记$p_s=0$的点；(c) 复位脉冲数$k$扫描，星号为实测网格与$k=3$的组合；(d) 5自旋Max-Cut重放中三种链路模型下首次命中基态的试验累积比例 (8次试验，同种子)；理想sigmoid与实测6-bit网格的曲线完全重合，前者以浅灰宽线衬底显示；(e) 误读概率的求解容限及其与实测判决失调的距离，橙色竖线分别为现有网表口径与参考端去耦口径下的实测误读率。
...
把失调剖面注入求解器需要一个每行都被占满的实例，否则多数行的淬火失调不参与耦合。为此取$n=64$的两个实例逐行映射 (一行一自旋)：植入划分实例的最优解由构造给出，配对的受挫实例与之共享边集与$|J|$、仅符号不同，故两者之差只反映符号结构。未补偿时的代价随阵列高度急剧上升：$N=64$在受挫实例上恶化$1.87\times$ (区间$[1.39,2.54]$)，$N=128$达$16.6\times$，$N=256$时1000次试验零命中、最优能量也够不到参考值。$N=64$的代价还依赖景观符号结构——配对的植入实例上反而加速$0.87\times$ (区间$[0.76,0.98]$)，这与3.5.2节同源：IR失调是绝对附加场，其相对权重取决于中位$|h_i^\mathrm{eff}|$，远端行失调在$N=64/128/256$下分别占中位局部场的$18\%$、$35\%$与$65\%$，恰好给出观测到的损伤次序。

补偿沿用写DAC本身：每行预置一个补偿码 ($N=64$最大需12个码、$N=256$需41个码，均在6-bit量程内)，预畸变把随行号累积的单边斜坡换成幅度与$N$无关的双边锯齿，残余不超过半个LSB。回灌结果在满阵上比14自旋的初判更强：两个实例、三种阵列高度、含与不含6-bit网格的全部配置，退化比都落在$0.83\sim1.14\times$且区间含$1$ (1000次试验)，末态平均磁化$|m|\leq 0.014$、与无失调基线不可分辨，而未补偿时磁化偏置随$N$增至$0.378$。补偿步进也无需精细：把量程按3.5.2节放宽到$\pm10V_T$后即便只用4 bit (残余$0.67V_T$、较$N=64$斜坡幅度的一半还大一个量级)，恢复依然完整，且所需码数自$N=256$的$41/63$降到$4/15$——以分辨率换量程与逐行预畸变彼此相容，无需另设细步进的偏置码路径。既有电路资源即可消除IR压降带来的失调，它并不构成工艺限制，代价是每行一次校准写码。$N=256$以上阵列的制约因此不在求解性能，而在补偿码预算与$1.044\,\mathrm{V}$的驱动裕度，进一步扩展需要更宽的金属或分段驱动，属于版图层的后续工作。
...
能量核算之外，读出通路的判决裕度才是本节暴露出的最紧约束。上述读出在标称器件下判决正确，但器件失配把这一结论推翻：对该StrongARM作Pelgrom失配蒙特卡洛 ($N=120$，sky130 tt)，输入折合失调$\sigma_\mathrm{off}=18.5\,\mathrm{mV}$，即$0.79V_T$、双态分离$34.3\,\mathrm{mV}$的$0.54$倍，已超过AP态$14.3\,\mathrm{mV}$的静态裕度本身。判决门限还依赖参考端的去耦状态：现有网表的参考端无电容，两态阻抗不等使kickback非对称地推动输入、恰好加固正确判决，去耦后该助力消失。两种口径给出的单次误读概率相差两个数量级 ($8.2\times10^{-4}$对$0.18$)，但结论在两者下一致——把误读作为翻转通道回灌求解器，乐观口径在$\mathrm{G1}$上即退化$5.87\times$ (区间$[4.17,8.72]$、与基线不重叠)，保守口径下200次试验零命中。该通道的容限同样随规模收紧：14自旋实例可容忍约$10^{-3}$的误读率，$\mathrm{G1}$只容忍约$10^{-5}$，与3.5.2节钳位量程的$N\cdot T$标度同源。与3.4.2节“循环噪声最被容忍”形成的对照很尖锐：$\sigma_\mathrm{C2C}=2$在$\mathrm{G1}$上仅$1.04\times$，而$8\times10^{-4}$的每次读错误已是$5.87\times$。物理通道比该模型更不利——失配是静态的，$\sigma_\mathrm{off}=18.5\,\mathrm{mV}$意味着约$13\%$与$23\%$的单元在两个状态下每次读都错，构成固定的缺陷图而非独立同分布噪声。要把$\sigma_\mathrm{off}$压到$\mathrm{G1}$所需的$3.3\,\mathrm{mV}$需$5.6\times$的改善，自调零结构在同流程下的$2.36\times$不足以独力达成，须与抬高读轨、把读出共模移入NMOS输入管适用区间等增大裕度的措施并用。读出通路因此取代写通路成为阵列规模下的首要电路约束。

![](figs/Chapter03_local_11.png)

图3.11 写线IR压降、更新能量核算与投影修正。(a) met2列写线的逐行驱动失调 (以$V_T$归一，双对数坐标)，标出$N=16/64/256$远端行的失调值，虚线为$N=64$经6-bit逐行预畸变后的残余；(b) 逐行占满的$64$自旋阵列上失调注入前后的$\mathrm{TTS}_{99}$比值 (1000次试验)：实心柱为未补偿、斜纹柱为逐行预畸变，两组分别取受挫实例与配对的植入划分实例，$N=256$的受挫未补偿点无命中；(c) 每次更新能量随复位脉冲数$k$的逐项分解 (功率门控口径的堆叠柱)，虚线为常开口径合计；(d) 以端到端参数 ($k=3$、门控口径) 修正后的G22硬件投影，sMTJ端到端点相对器件级点沿两轴各移约$18.7\times$。
...
本节结论的适用口径如下。全部CMOS数字为sky130原理图级仿真，主口径取tt角，五个工艺角的散布见表3.9注与3.5.4节，未含版图寄生 (写线电阻除外)；失配统计只作用于读出比较器 (3.5.4节)，写通路的失配未扫描，其影响预期与该处同向；缓冲器与使能门取首轮可工作尺寸，未经速度或功耗优化，2.87 mW静态功率与9.0 ns建立时间 (tt角) 应视为该拓扑在此尺寸下的代表值而非极限；复位脉冲需要反极性驱动，以同幅值正极性脉冲的欧姆耗散作其能量代理，桥式驱动电路未实现；复位脉冲序列的统计已用宏自旋LLG标定 (见3.5.2节)，该标定属仿真而非实测、限于单一工作点，回跳概率对工作点的依赖未扫描；误读通道以独立同分布的翻转建模，而失配导致的实际误读是静态缺陷图，该建模低估其危害 (见3.5.4节)；局部场$h_i^\mathrm{eff}$的数字求和电路未计入能量核算，该项在表3.8的sMTJ与CMOS p-bit行中同样缺席、两行之间口径一致，而FPGA行的nJ级能耗与CPU行的端到端实测已含权重求和，sMTJ相对这两行的能耗差距因此被相应低估。绝对数值受上述近似影响，以$V_T$归一的比值与各项排序则是可迁移的结论。本节全部数据由仓库`eda/`目录下的测试台脚本生成，每个数字对应一份带随机种子记录的结果文件，图3.9至图3.11可由`figs_make.py`直接重建。
